...
# =================== CONFIGURATION ===================
SYMBOL = "THETA_USDT"  # THETA/USDT futures symbol
# =====================================================
...
def download_historical_data(symbol, limit=3000):
    ...
    while len(all_candles) < limit and current_start < now:
        ...
        params = {
            "symbol": symbol,  # Use original format THETA_USDT for futures
            "interval": "Min15",
            "start": current_start,
            "end": now,
...
def download_spot_historical_data(symbol, limit=300):
    """Fallback: Download from spot API if futures API fails"""
    print(f"📥 Trying spot API for {symbol}...")
    
    url = "https://api.mexc.com/api/v3/klines"
    spot_symbol = symbol.replace("_", "")  # Convert THETA_USDT to THETAUSDT
    params = {
        "symbol": spot_symbol,
        "interval": "1m",
        "limit": limit
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Downloaded {len(data)} historical candles from spot API")
            return data
        else:
            print(f"❌ Spot API also failed: {response.status_code}")
            return []
    except Exception as e:
